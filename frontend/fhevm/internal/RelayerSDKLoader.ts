      ...
      new Promise((resolve, reject) => {
        ...
        script.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            reject(
              new Error(
                `RelayerSDKLoader: Relayer SDK script loaded from ${src}, but window.relayerSDK is invalid.`
              )
            );
            return;
          }
        ...
        script.onerror = () => {
          reject(new Error(`RelayerSDKLoader: Failed to load Relayer SDK from ${src}`));
        };
